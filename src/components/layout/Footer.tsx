...
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="w-full bg-[#EFE9DC] text-slate-900 border-t border-black/10 pt-20 pb-16 px-6 sm:px-8">
      <div className="max-w-[var(--container-wide-width)] mx-auto flex flex-col gap-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 sm:gap-8">
          ...
          <div className="flex flex-col gap-4 md:col-span-1">
            <div className="flex items-center gap-3">
              <div className="relative w-10 h-10 flex-shrink-0 rounded-xl overflow-hidden p-0.5 bg-gradient-to-br from-[var(--color-accent-gold)]/30 to-white border border-[var(--color-accent-gold)]/50 shadow-sm">
                <Image
                  src={logoAsset.path}
                  alt={logoAsset.alt}
                  width={40}
            ...
            <p className="text-xs text-slate-700 font-normal leading-relaxed max-w-xs mt-1">
              ...
              {socialsConfig.channels.map((channel) => (
                <li key={channel.id}>
                  {channel.url ? (
                    ...
                    <span className="opacity-80 py-0.5 inline-block">{channel.platform}</span>
                  )}
                </li>
              ))}
            </ul>
        ...
        {/* Bottom copyright */}
        <div className="pt-8 border-t border-black/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left text-xs text-slate-700 font-mono font-medium">
          <span>© {currentYear} {brandConfig.name}. Todos los derechos reservados.</span>
          <span className="text-[var(--color-accent-gold)] font-bold">SANTIAGO, CHILE</span>
        </div>
      </div>
    </footer>
  );
